...
export const decactivePost = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Post visibility has been successfully updated." });
  } catch (error) {
    console.log(error.message);
    return res.status(500).json({ errMessage: error.message });
  }
 ...
 * @param {import('express').Response} res
 */
export const getAllBuyLaterItems = async (req, res) => {
  try {
    ...
    const buyLaterItems = await sql`
           SELECT buylater_id, buylater.category, buylater.ad_id, title, price, images, condition, region, town, users.phone, users.phone2
           FROM buylater
           JOIN mobilephones ON mobilephones.mobile_id = buylater.ad_id
           JOIN users ON users.user_id = mobilephones.user_id
           WHERE buylater.user_id = ${userID}
        `;

    if (buyLaterItems.length === 0 || !buyLaterItems) {
      return res.status(200).json([]);
    }

    console.log(buyLaterItems);

    res.status(200).json(buyLaterItems);
  } catch (error) {
    res.status(400).json([]);
  }
};
 ...
 * @param {import('express').Response} res
 */
export const removeAllBuyLater = async (req, res) => {
  try {
    const userID = req.userData.userID.user_id;
...
// This removes each buy later item on a single click
/**
 * @param {import('express').Request} req
 * @param {import('express').Response} res
 */
export const removeOneBuyLater = async (req, res) => {
  try {
    const buylater_id = Number(req.params.id);
    const userID = req.userData.userID.user_id;
    const remove = await sql`
          DELETE FROM buylater
          WHERE buylater_id =${buylater_id} AND user_id = ${userID}
          RETURNING *
        `;
    if (remove.length === 0)
      return res.status(200).json({ message: "No data found" });
