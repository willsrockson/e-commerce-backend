...
import { SubCategoryTable } from '../categories-schema.js'
...
export const MobilePhoneTable = pgTable("mobile_phones", {
  ...
  battery_size: text('battery_size').array().notNull().default(sql`ARRAY[]::text[]`),
  screen_size: text('screen_size').array().notNull().default(sql`ARRAY[]::text[]`),
...
export const MobilePhoneGeneral = pgTable("mobile_phones_general", {
    ...
    verification_status: text("verification_status").array().notNull().default(sql`ARRAY['verified', 'unverified']::text[]`),
    sub_category_id: integer('sub_category_id').references(()=> SubCategoryTable.sub_category_id, {onDelete: 'cascade'} ).notNull(),
    ...timestamps
});